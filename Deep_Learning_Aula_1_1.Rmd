...
# deixando os dados menores, porque em redes neurais é preciso trabalhar com dados mais proximos de zero
# porque redes neurais não são perfomaticas quando os numeros estão alto.
data <- scale(data)  

#Train Test Split, dividindo a base de dados em base de teste e treino.
train_test_split_index <- 0.8 * nrow(data)

train <- data.frame(data[1:train_test_split_index,])
test <- data.frame(data[(train_test_split_index+1): nrow(data),]) # nada depois da virgula porque queremos de toda as colunas 
...
initializeParameters <- function(X, layer_size){
    ...
    n_h <- layer_size$n_h
    n_y <- layer_size$n_y
    # runif chuta uma distribuição uniforme
...
trainModel <- function(X, y, num_iteration, hidden_neurons, lr){
    ...
    layer_size <- getLayerSize(X, y, hidden_neurons) # cria arquitetura 
    init_params <- initializeParameters(X, layer_size) # chuta os valores dos pesos 
    
    cost_history <- c() 

    for (i in 1:num_iteration) {
				# fwd é onde verifica ativação 
